...
public abstract class GeneticAlg {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(GeneticAlg.class);
    ...
    //__________________________________________________________________________
        ...
        /**
     *Applies the evolution algorithm to a population,and returns 
     * the resulting evolved population
     */
    public   Individual[]  evolve (Individual[] population){
        
        if (logger.isDebugEnabled()) {
          logger.debug("evolve(Individual[]) - start"); //$NON-NLS-1$
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("evolve(Individual[]) - end"); //$NON-NLS-1$
        }
     ...
     */
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("load(Memoria) - start"); //$NON-NLS-1$
        }
        this.population = memory.loadPoblacion(population[0].chromosomeSize());
        boolean success = population != null;
        currentGeneration = memory.getCurrentGeneration();
        currentIndividual = -1;
        //        currentMatch = memory.loadIteration ();
        if (logger.isDebugEnabled()) {
            logger.debug("load(Memoria) - end"); //$NON-NLS-1$
        }
        return success;
    }
    ...
    public void initPopulation() {
        ...
        for (int i = 0; i < population.length; ++i) {
            ...
            population[i] = new IndividualV1(false, NonLinearFitness.class);
        }
        this.chromosomeSize = population[0].chromosomeSize();
    }

    public void setCurrentIndividual(int currentIndividual) {
