...
/**
 * When this secondary state is activated the bot will collect the closest health
...
public class PickupHealth extends SecondaryState{
	 ...
	 */
	private static final Logger logger = Logger.getLogger(PickupHealth.class);
            ...
            if (info.getHealth() < 100) {
                if (healthPackMap.isEmpty()) {
                    if (healthVialMap.isEmpty()) {
                        throw new SubStatusException ("No existen items de vida visibles");
                    }
                    else {
