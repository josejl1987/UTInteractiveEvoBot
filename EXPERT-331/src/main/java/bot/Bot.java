...
public class Bot extends UT2004BotModuleController {
    ...
    // 6 = Oeste, 7 = Nor-oeste
    /**
     * Vector containing all the rays (
     *
     * @see AutoTraceRay.class). To make it easier, we will use constants to
     * reference certain rays in the vector. NORTH RAY = 0 NORTHEAST RAY = 1
     * EAST RAY = 2 SOUTHEAST RAY = 3 SOUTH RAY = 4 SOUTHWEST RAY = 5 WEST RAY =
     * 6 NORTHWEST RAY = 7
     */
    ...
    /**
     * Identifies the Disabled sub state in the secondary states vector (note
     * that it is null, hence IT CAN'T BE USED)
     */
    ...
    //__________________________________________________________________________
    /**
     * It gets triggered whenever we gain adrenaline (by killing or picking it
     * up)
     *
    ...
    public static void main(String args[]) throws PogamutException, Exception {
        ...
        UT2004BotRunner bot = new UT2004BotRunner(Bot.class, "T800").setMain(true);
        System.out.println("Bot experto arranque");
        bot.setName("Prueba");
   //     bot.setLogLevel(Level.ALL);
        bot.startAgent();
